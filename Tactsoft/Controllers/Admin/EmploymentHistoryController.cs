...
namespace Tactsoft.Controllers.Admin
{
    public class EmploymentHistoryController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, EmploymentHistory employmentHistory)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    var emp = await _employmentHistoryService.FindAsync(id);
        ...
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Delete(int id, EmploymentHistory employmentHistory)
        {
            var serv = await _employmentHistoryService.FindAsync(x => x.Id == id, x => x.Employee,x => x.Designation);
            if (serv != null)
            {
                await _employmentHistoryService.DeleteAsync(employmentHistory);
